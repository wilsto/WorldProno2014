...
.directive('popOver', function ($compile) {
        var itemsTemplate = "<table  class='table table-responsive table-condensed'><tr><td class='group'>Grp</td><td class='group'>Equipes</td><td class='group'>Pts</td><td class='group'>Réel</td><td class='group'>Prono</td></tr><tr ng-repeat='item in items' ng-class='{success:item.points,warning:item.points ===0}'><td>{{item.group}}</td><td>{{item.countries}}</td><td class='Pts'>{{item.points}}</td><td>{{item.scorereel}}</td><td>{{item.score}}</td></tr></table>";
        	...
        	_.each(dataPlayer.groupsMatches, function(groupsMatches, group){	// pour chaque groupe
        		...
        		_.each(groupsMatches.matches, function(match){	// pour chaque match
					...
					if(realMatch[0].score.length === 0 || realMatch[1].score.length === 0 ){
						...
						switch( matchNb) {
						  case 0 :
						  case 1 : var tour ='tour1'; break;
						  case 2 :
						  case 3 : var tour ='tour2'; break;
						  case 4 :
						  case 5 : var tour ='tour3'; break;
						}

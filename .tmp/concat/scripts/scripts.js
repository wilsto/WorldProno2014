  ...
  function ($scope, $http, $location, PronoFactory, Auth) {
    ...
    $scope.initPronos = function () {
      $http.get('/api/fifaMatchs').success(function (fifaMatchs) {
        ...
        $scope.real = $location.path() === '/worldcup/Mondial' ? 'Mondial' : $scope.user.username;
...
angular.module('worldProno2014App').controller('NavbarCtrl', [
  ...
  function ($rootScope, $scope, $location, Auth) {
    ...
    $scope.reset = function () {
      bootbox.confirm('Etes-vous sur de vouloir r\xe9-initialiser votre pronostic ?', function (result) {
        if (result) {
          $scope.resetPronos();
        }
      });
...
angular.module('worldProno2014App').directive('popOver', [
  ...
  'PronoFactory',
  'Auth',
  function ($scope, $http, PronoFactory, Auth) {
    var points = {
        result: 3,
        score: 1,
        qualif: 2,
